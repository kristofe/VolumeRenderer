...
GameID RendererObjectFactory::CreateUnitPlaneMesh()
{
	...
	m.mVertices = new Vector3[m.mVertexCount];
	m.mNormals = new Vector3[m.mNormalCount];
	m.mColors = new Color[m.mColorCount];
	m.mUV = new Vector2[m.mUVCount];
...
GameID RendererObjectFactory::CreateGridMesh(std::string name, int widthInCells, int heightInCells, float width, float height)
{
	...
	m.mColors = new Color[m.mColorCount];
	m.mUV = new Vector2[m.mUVCount];
	m.mUV2 = new Vector2[m.mUV2Count];
...
////////////////////////////////////////////////////////////////////////////////
GameID RendererObjectFactory::CreateQuadPoolMesh(std::string name, int quadCount, const Vector3& quadSize)
{   
	GameID rid = RendererObjectFactory::CreateMesh(name);
	Mesh& m = RendererObjectFactory::GetMesh(rid);
