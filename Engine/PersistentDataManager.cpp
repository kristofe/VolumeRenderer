...
Vector3 PersistentData::GetVector3Child(const char* name)
{
	Vector3 v;
	std::string n = name;
	PersistentData* c = GetUniqueChild(n);
...
void PersistentData::AddVector4Child(Vector4& v, const char* name)
{
	PersistentData* c = new PersistentData(name);
	c->SetProperty("x",v.x);
	c->SetProperty("y",v.y);
...
Vector4 PersistentData::GetVector4Child(const char* name)
{
	Vector4 v;
	std::string n = name;
	PersistentData* c = GetUniqueChild(n);
